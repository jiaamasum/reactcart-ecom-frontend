...
import { apiGetOrder, type OrderView } from "@/lib/api"
...
export default function OrderConfirmationPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        if (params?.id) {
          const o = await apiGetOrder(String(params.id))
          setOrder(o)
        }
      } catch { setOrder(null) }
  ...
  const numberFormatted = (order as any).orderNumberFormatted || (order as any).orderNumber || null
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="font-semibold">{numberFormatted ? `#${numberFormatted}` : order.id}</p>
            </div>
            <div>
